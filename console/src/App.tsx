...
export default function App() {
  ...
  async function submitPrompt(event: FormEvent) {
    ...
    try {
      if (isLive) {
        ...
        setTurns((current) => [
          ...current,
          { id: crypto.randomUUID(), role: "mise", text: readableAgentResponse(result.response) },
        ]);
        await refreshLive();
        return;
      }
  ...
  async function runDemoAgent(text: string) {
    ...
    if (awaitingClarification) {
      ...
      setProposed(structuredClone(demoProposedChange));
      setActivePlan(structuredClone(demoProposedChange.plan));
      setHistory((current) => ({ ...current, plans: [structuredClone(demoProposedChange.plan), ...current.plans] }));
      setTurns((current) => [
        ...current,
        {
          id: crypto.randomUUID(),
          role: "mise",
          text: `Understood. ${demoProposedChange.interpretation} I generated an inspectable saved plan. No POS write has occurred.`,
        },
      ]);
      return;
    }
    setTurns((current) => [
      ...current,
      {
        id: crypto.randomUUID(),
        role: "mise",
        text: `I interpreted: “${text}”. The current demo already has a governed rollout proposal; review the exact plan before approving another change.`,
      },
    ]);
  }
  ...
  async function approvePlan() {
    ...
    if (isLive && !accessCode) {
      setNotice("Enter the operator demo access code before approving a plan.");
      return;
    }
    setBusy(true);
    setNotice(null);
    try {
      if (isLive) {
        ...
        setTurns((current) => [...current, { id: crypto.randomUUID(), role: "mise", tone: "success", text: "Exact plan hash approved. Revision 12 now defines desired state; rollout has not started yet." }]);
      }
    } catch (error) {
      setNotice(errorMessage(error));
    } finally {
  ...
  async function startApply() {
    if (!activePlan) return;
    if (isLive && !accessCode) {
      setNotice("Enter the operator demo access code before applying a plan.");
      return;
    }
    setBusy(true);
    setNotice(null);
    try {
      if (isLive) {
        const started = await client.apply(activePlan.plan_id, accessCode);
        setRollout(started);
        return;
      }
      ...
      demoTimer.current = window.setInterval(() => {
        ...
        setRollout((current) => current ? { ...current, ...progress, failures: progress.status === "partial" ? structuredClone(demoPartialRollout.failures) : current.failures, updated_at: new Date().toISOString() } : current);
  ...
  async function retryFailures() {
    ...
    if (isLive) {
      if (!accessCode) {
        setNotice("Enter the operator demo access code before retrying.");
        return;
      }
      try {
  ...
  function actOnDrift(driftId: string, action: "remediate" | "override" | "investigate") {
    if (action === "remediate") {
      ...
      setPage("changes");
      setTurns((current) => [...current, { id: crypto.randomUUID(), role: "mise", text: "I prepared a remediation path for the selected drift. It still requires a new deterministic plan and explicit approval before any POS write." }]);
        ...
        <header className="topbar">
          ...
          <div className="topbar-actions">
            ...
            <label className="access-control">
              ...
              <input type="password" value={accessCode} onChange={(event) => setAccessCode(event.target.value)} placeholder={isLive ? "Required for writes" : "Not required in demo"} />
            </label>
          </div>
        </header>

        {!isLive && <div className="demo-banner"><strong>LOCAL DEMO DATA</strong><span>This mode demonstrates the locked product behavior; it is not presented as live Square state.</span></div>}
        {notice && <div className="notice" role="status"><span>{notice}</span><button onClick={() => setNotice(null)} aria-label="Dismiss">×</button></div>}

        {page === "overview" && (
          <OverviewPage estate={estate} rollout={rollout} convergence={convergence} openDrift={unresolvedDrift} snapshotTime={snapshotTime} onOpenChanges={() => setPage("changes")} />
        )}
        {page === "changes" && (
          <ChangesPage
            turns={turns}
    ...
    <section className="page-grid">
      <div className="hero-card">
        <div>
          ...
          <p>{estate.has_desired_state ? "Mise separates approved policy from live convergence. Deviations remain visible until remediated or explicitly overridden." : "The first import is evidence of reality, not policy. Review the estate, sanitize differences, then approve the desired configuration when ready."}</p>
        </div>
...
function StructuredConfig({ proposed }: { proposed: ProposedChange }) {
  return <article className="panel"><PanelHeading title="Structured intent" detail="Generated by trusted code from typed agent output" /><p>{proposed.interpretation}</p><pre className="config-preview">{proposed.configPreview}</pre></article>;
}
  ...
  return (
    <article className="panel plan-panel">
      ...
      <div className="blast-grid">
        <Blast value={String(proposed?.target_count ?? summary.targeted_locations ?? "—")} label="locations" />
        <Blast value={String(proposed?.states ?? summary.states ?? "—")} label="states" />
        <Blast value={String(proposed?.exception_count ?? summary.exception_locations ?? "—")} label="exceptions" />
        <Blast value={String(proposed?.change_count ?? Number(summary.to_create ?? 0) + Number(summary.to_update ?? 0))} label="changes" />
      </div>
      <div className="hash-row"><span>Exact plan SHA-256</span><code>{plan.plan_hash}</code></div>
      <div className="plan-actions">
        {!approved ? <button className="button primary" onClick={onApprove}>Approve exact plan</button> : <><span className="status-pill success">Approved</span><button className="button danger" onClick={onApply}>Apply approved plan</button></>}
      </div>
    </article>
  );
}
...
function DriftPage({ drift, onAction }: { drift: DriftRecord[]; onAction: (id: string, action: "remediate" | "override" | "investigate") => void }) {
  return <section className="page-grid"><div className="section-intro"><div><p className="eyebrow">Approved policy vs. observed reality</p><h2>Configuration drift</h2><p>Every deviation is compared against the location’s applicable policy, including approved exceptions.</p></div></div><div className="drift-list">{drift.map((item) => <article className="panel drift-card" key={item.drift_id}><div className="drift-head"><div><span className={`risk ${item.impact}`}>{item.impact} impact</span><h3>{item.location}</h3><p>{item.resource}</p></div><span className={`status-pill ${item.status}`}>{item.status.replaceAll("_", " ")}</span></div><div className="compare-grid"><div><span>Expected</span><strong>{item.expected}</strong></div><div><span>Observed</span><strong>{item.actual}</strong></div></div><p className="muted">{item.rationale}</p><div className="drift-actions"><button className="button primary" onClick={() => onAction(item.drift_id, "remediate")}>Review remediation plan</button><button className="button secondary" onClick={() => onAction(item.drift_id, "override")}>Propose new exception</button><button className="text-button" onClick={() => onAction(item.drift_id, "investigate")}>Investigate</button></div></article>)}</div></section>;
}

function LocationsPage({ locations, estate }: { locations: LocationRecord[]; estate: EstateResponse }) {
  return <section className="page-grid"><div className="section-intro"><div><p className="eyebrow">Representative branch inventory</p><h2>Locations & targeting groups</h2><p>The demo displays representative locations while the estate summary models 200 branches. Targeting supports branches, geography, and reusable groups/tags.</p></div></div><article className="panel table-panel"><table><thead><tr><th>Location</th><th>Geography</th><th>Group</th><th>Applicable status</th><th>Exception</th></tr></thead><tbody>{locations.map((location) => <tr key={location.id}><td><strong>{location.name}</strong><span>{location.id}</span></td><td>{location.city}, {location.state}</td><td><code>{location.group}</code></td><td><span className={`status-pill ${location.status}`}>{location.status}</span></td><td>{location.exception ?? "—"}</td></tr>)}</tbody></table></article><p className="muted">Desired policy: {estate.desired_revision?.display_name ?? "Not established yet — current differences are observed state only."}</p></section>;
}
...
function Blast({ value, label }: { value: string; label: string }) { return <div className="blast"><strong>{value}</strong><span>{label}</span></div>; }
