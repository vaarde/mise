...
export default function LiveApp() {
  ...
  const refreshLive = useCallback(async (options: { conformance?: boolean } = {}) => {
    ...
    try {
      ...
      setConnected(true);
      setNotice((current) => {
        if (current?.tone === "critical") return null;
        if (current?.tone === "attention" && nextEstate.latest_rollout && isTerminalRolloutStatus(nextEstate.latest_rollout.status)) return null;
        return current;
      });
  ...
  async function refreshRollout(rolloutId: string) {
    try {
      ...
      if (isTerminalRolloutStatus(next.status)) {
        setNotice((current) => current?.tone === "attention" ? null : current);
  ...
  async function submitPrompt(event?: FormEvent, override?: string) {
    ...
    try {
      ...
      replacePending({ role: "mise", kind: "failure", text: `I couldn't prepare a plan for that. ${sentence(message)} Nothing was approved or changed.`, retry: text });
    } finally {
      setAgentBusy(false);
    }
  }
  ...
  async function runProtected(action: Protected, code: string) {
    ...
    try {
      if (action.kind === "approve") {
        ...
        setDialog(null);
        toast("positive", "Plan approved", isPolicyOnly(activePlan)
  ...
  const nav: Array<{ key: ConsolePage; label: string; icon: IconName; count?: string; tone?: "attention" | "ok" }> = [
    ...
    { key: "drift", label: "Differences", icon: "differences", count: differences ? String(differences.length) : undefined, tone: differences ? (differences.length ? "attention" : "ok") : undefined },
  ...
  return (
    <>
      <div className="env-strip" role="note">
        ...
        <span className="right"><span className={`dot ${loading ? "" : connected ? "on" : "off"}`} aria-hidden />{loading ? "Connecting..." : connected ? (MOCK ? "Mock API" : "Live data") : "Disconnected"}</span>
      </div>
      <div className="shell">
        <aside className="side">
          <div className="account"><span className="brand-mark" aria-hidden>M</span><span><strong>Mise</strong><span>{orgName || "Operations control"}</span></span></div>
          <nav className="nav" aria-label="Primary">
            {nav.map((item) => (
              <button key={item.key} type="button" className="nav-link" aria-current={page === item.key ? "page" : undefined} onClick={() => go(item.key)}>
                <Icon name={item.icon} /><span>{item.label}</span>{item.count && <span className={`nav-count ${item.tone ?? ""}`}>{item.count}</span>}
              </button>
            ))}
          </nav>
          <div className="side-foot"><div className="conn" role="status"><span className={`dot ${loading ? "" : connected ? "on" : "off"}`} aria-hidden /><span><strong>{loading ? "Connecting..." : connected ? "Connected" : "Disconnected"}</strong><br />{MOCK ? "Local mock API, sample data" : "Live data only, never samples"}</span></div></div>
        </aside>

        <main className="main">
          <div className="topbar">
            <SearchBox estate={estate} plans={allPlans} differences={differences ?? []} onLocation={openLocation} onPlan={openPlan} onDifferences={() => go("drift")} />
            <div className="topbar-right">
              <button type="button" className="switch" role="switch" aria-checked={Boolean(accessCode)} onClick={() => setDialog({ kind: "unlock" })} title={accessCode ? "Updates are turned on for this tab" : "Turn on updates to approve plans or send them to Square"}>Updates <span className="track" aria-hidden /></button>
              <button className={`icon-btn ${loading ? "spin" : ""}`} onClick={() => void refreshLive({ conformance: true })} disabled={loading} aria-label="Refresh live data"><Icon name="refresh" /></button>
            </div>
          </div>

          {notice && <div className="notice-wrap"><Banner tone={notice.tone} onDismiss={() => setNotice(null)}><span>{notice.text}</span></Banner></div>}

          {page === "overview" && <OverviewPage connected={connected} loading={loading} estate={estate} conformance={conformance} rollout={rollout} rolloutPlan={rolloutPlan} revisionPlan={revisionPlan} plans={allPlans} rollouts={history.rollouts} next={next} onGo={go} onOpenPlan={openPlan} onOpenLocation={openLocation} />}
          {page === "changes" && <ChangesPage estate={estate} turns={turns} prompt={prompt} setPrompt={setPrompt} agentBusy={agentBusy} decision={decision} onCancelDecision={() => { setDecision(null); setPrompt(""); }} onSubmit={submitPrompt} plan={activePlan} phase={phase} rollout={activeRollout} plans={allPlans} rollouts={history.rollouts} latestRollout={rollout} revisions={history.revisions} unlocked={Boolean(accessCode)} actionBusy={actionBusy} onSelectPlan={(planId) => setActivePlanId(planId)} onApprove={() => setDialog({ kind: "approve" })} onApply={() => setDialog({ kind: "apply" })} onRetry={() => setDialog({ kind: "retry" })} onOpenDifferences={() => { go("drift"); void checkConformance(); }} onHome={() => go("overview")} onRetryRequest={(text) => void submitPrompt(undefined, text)} onTurnShown={(id) => setTurns((current) => current.map((turn) => (turn.id === id ? { ...turn, fresh: false } : turn)))} planArrivedId={planArrivedId} onPlanArrivalShown={() => setPlanArrivedId(null)} />}
          {page === "drift" && <DifferencesPage estate={estate} conformance={conformance} audit={audit} onCheck={() => void checkConformance()} onLoadAudit={() => void loadAudit()} onDecide={decide} onHome={() => go("overview")} />}
          {page === "locations" && !locationId && <LocationsPage estate={estate} conformance={conformance} verified={verified} onOpenLocation={openLocation} onHome={() => go("overview")} />}
          {page === "locations" && locationId && <LocationDetailPage locationId={locationId} estate={estate} conformance={conformance} verified={verified} plans={allPlans} rollouts={history.rollouts} latestRollout={rollout} onBack={() => setLocationId(null)} onGo={go} onOpenPlan={openPlan} />}
        </main>
      </div>

      {dialog?.kind === "unlock" && <UnlockDialog accessCode={accessCode} onSave={(code) => { setAccessCode(code); setDialog(null); toast(code ? "positive" : "info", code ? "Updates turned on" : "Updates turned off", code ? "You can now approve plans and send them to Square from this tab." : "This tab is read-only again."); }} onClose={() => setDialog(null)} />}
      {dialog && dialog.kind !== "unlock" && activePlan && <ConfirmDialog action={dialog} plan={activePlan} targets={activePlan.target_location_ids?.length ?? null} locationCount={estate.observed_estate?.location_count ?? 0} accessCode={accessCode} busy={actionBusy} onConfirm={(code) => { setAccessCode(code); void runProtected(dialog, code); }} onOpen={() => setDialogError(null)} onClose={() => { if (!actionBusy) { setDialog(null); setDialogError(null); } }} error={dialogError} />}
...
function SearchBox(props: {
  ...
  if (q) {
    for (const location of props.estate.observed_estate?.locations ?? []) {
      if (`${location.name} ${location.state ?? ""} ${location.metadata?.city ?? ""} ${location.id}`.toLowerCase().includes(q)) results.push({ group: "Locations", label: location.name, meta: location.state ?? "", run: () => props.onLocation(location.id) });
    }
    for (const plan of props.plans) {
      if (`${plan.title ?? ""} ${plan.plan_id}`.toLowerCase().includes(q)) results.push({ group: "Plans", label: plan.title || plan.plan_id, meta: formatTime(plan.created_at), run: () => props.onPlan(plan.plan_id) });
    }
    for (const item of props.differences) {
      if (`${item.resourceLabel} ${item.property}`.toLowerCase().includes(q)) results.push({ group: "Differences", label: `${item.resourceLabel} · ${item.property}`, meta: `${item.approved} → ${item.squareNow}`, run: props.onDifferences });
    }
  }
  ...
  return (
    <div className="search" onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node)) setOpen(false); }}>
      <Icon name="search" />
      <input ref={inputRef} type="search" role="combobox" aria-expanded={open && Boolean(q)} aria-controls={listId} aria-activedescendant={open && shown[active] ? `search-opt-${active}` : undefined} aria-label="Search locations, plans and differences" placeholder="Search locations, plans, differences" value={query} onChange={(event) => { setQuery(event.target.value); setActive(0); setOpen(true); }} onFocus={() => setOpen(true)} onKeyDown={(event) => {
      ...
      {open && q && <div className="search-pop" id={listId} role="listbox">
        {shown.length === 0 && <div className="none">No matches for “{query}”</div>}
        {shown.map((result, index) => <div key={`${result.group}-${index}`}>
          {(index === 0 || shown[index - 1]!.group !== result.group) && <h4>{result.group}</h4>}
          <button id={`search-opt-${index}`} type="button" role="option" aria-selected={index === active} onMouseEnter={() => setActive(index)} onClick={() => pick(index)}><span>{result.label}</span><span>{result.meta}</span></button>
        </div>)}
      </div>}
    </div>
  );
...
function UnlockDialog({ accessCode, onSave, onClose }: { accessCode: string; onSave: (code: string) => void; onClose: () => void }) {
  ...
  return (
    <Dialog title={accessCode ? "Updates are on" : "Turn on updates"} onClose={onClose} footer={<>{accessCode && <button className="btn" onClick={() => onSave("")}>Turn off</button>}<button className="btn" onClick={onClose}>Cancel</button><button className="btn primary" onClick={() => onSave(code.trim())} disabled={!code.trim()}>Turn on updates</button></>}>
      ...
      <form onSubmit={(event) => { event.preventDefault(); if (code.trim()) onSave(code.trim()); }}><label className="field"><span>Operator code</span><input data-autofocus type="password" autoComplete="off" value={code} onChange={(event) => setCode(event.target.value)} /></label></form>
    </Dialog>
  );
}
...
function ConfirmDialog(props: {
  ...
  const copy = {
    approve: { heading: "Approve this plan?", button: "Approve plan", busy: "Approving", body: policyOnly ? `“${title}” will become your approved setup. Square already has these values, so nothing will be sent.` : `“${title}” will become your approved setup. Approving does not change Square. You send it to Square as a separate step.` },
    apply: { heading: "Send this plan to Square?", button: "Send to Square", busy: "Sending", body: `Mise will send exactly what was approved: ${writes.create} new, ${writes.update} changed and ${writes.remove} removed settings${props.targets !== null ? `, at ${props.targets} of ${props.locationCount} locations` : ""}. Then it reads Square back to confirm.` },
    retry: { heading: "Try sending again?", button: "Try again", busy: "Starting", body: "Mise will send the same approved plan again and check the result. It helps to look at Differences first so you know what Square has now." },
  }[props.action.kind];

  return (
    <Dialog title={copy.heading} onClose={props.onClose} footer={<><button className="btn" onClick={props.onClose} disabled={props.busy}>Cancel</button><button className={`btn ${props.action.kind === "approve" ? "primary" : "dark"}`} onClick={() => props.onConfirm(code.trim())} disabled={props.busy || !code.trim()} aria-busy={props.busy}>{props.busy ? <><Spinner />{copy.busy}</> : copy.button}</button></>}>
      <p>{copy.body}</p>
      <p className="muted" style={{ fontSize: 12 }}>Plan fingerprint <span className="mono">{props.plan.plan_hash.slice(0, 12)}...{props.plan.plan_hash.slice(-10)}</span></p>
      {props.error && <div className="callout critical" role="alert" style={{ margin: "0 0 12px" }}><div><b><Icon name="alert" size={14} />Not done</b><span>{props.error}</span></div></div>}
      {(!props.accessCode || props.error) && <label className="field"><span>Operator code</span><input data-autofocus type="password" autoComplete="off" value={code} onChange={(event) => setCode(event.target.value)} /></label>}
    </Dialog>
  );
}
...
function readableAgentResponse(value: unknown): string {
  ...
  for (const key of ["message", "text", "interpretation", "clarification_question"]) if (typeof record[key] === "string") return String(record[key]);
