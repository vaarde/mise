...
- Mise never deletes POS resources. `provider.Delete` exists on the interface but the engine has no caller and there is no `--destroy` flag yet. Safety-first default.
- Config files are parsed with `KnownFields(true)`. An unrecognized key is an error, because a silently ignored one (`location:` for `locations:`) plans and applies cleanly while doing something else entirely. `locations.yaml` is generated reference data and is skipped by the resource loader, not parsed as a resource file.
- A resource is identified by `type.name`, and declaring the same one twice is an error. Every stage downstream keys by that name, so a duplicate would win or lose silently depending on file walk order.
- A resource must resolve to at least one location. `config.ResolveLocations` refuses an empty result, `engine.checkWritable` refuses it again on a saved plan, and the Square adapter refuses it a third time — see the design decision below for why three.
...
- **An empty location scope is refused in three places, on purpose.** Square reads an object with no `present_at_location_ids` as present at *every* location, so "nowhere" and "everywhere" are the same bytes on the wire. A group filter with one typo used to widen a write to the whole account while the plan showed the scope shrinking to nothing. `config.ResolveLocations` catches it when the group resolves, `engine.checkWritable` catches it on a saved plan (which is a file, and a file can be edited), and `square.catalogObjectFor` catches it at the last moment before the request. Only the first has a good error message; the other two exist because the cost of missing this is an account-wide menu change nobody asked for.
- **The workspace records which POS account it belongs to.** `state.Identity` is `{provider, environment, account_id}`, stamped by `fetch` and `apply`, checked by `plan`, `apply` and `drift`. Provider IDs mean nothing outside the account that issued them: point a workspace at a second merchant and every recorded ID misses the live lookup, so plan takes the "deleted outside Mise, recreate it" branch for every resource and duplicates the entire configuration into the wrong account — successfully, with no error. `provider.AccountIdentifier` is the optional capability that names the account (Square uses the merchant ID, which every location already carries). An adapter that cannot name one degrades to checking platform and environment; state written before identities existed records nothing and is allowed through, then stamped on its next write.
- **A saved plan carries provenance, not just changes.** `cmd.SavedPlan` wraps the plan with the account, the state serial, a config digest, the Mise version and a timestamp, and `apply --plan` refuses a plan built for another account, computed against state that has since moved, or made from config that has since been edited. A plan file is a list of provider IDs and version tokens; executed anywhere else it is a set of instructions about objects that do not exist there. The format is versioned (`savedPlanFormat`), and pre-provenance plan files are refused rather than trusted.
- **State writes are atomic and checkpointed per wave.** `state.Save` writes to a temporary file, fsyncs, and renames — a crash leaves the old state or the new one, never a truncated file, because a truncated `state.json` means Mise no longer knows which live resources it created. `engine.ApplyOptions.Checkpoint` persists after each wave, so a crash costs the record of one wave rather than the whole run; a checkpoint that fails stops the apply, since continuing widens exactly the gap it exists to close. `State.Serial` counts saves and is what makes a stale saved plan detectable.
- **Adapters declare property schemas; the engine does the checking.** `provider.SchemaProvider` returns the accepted property names per resource type, and `engine.ValidateDeclared` runs before every plan. Without it a misspelled property is invisible end to end: the adapter's `passThrough` copies only names it recognizes, so the typo is dropped on the way out, the plan shows the change, the apply reports success, the POS never receives it, and the next plan proposes the same change forever. A typo'd `percentage` on a tax is worse — the tax goes up with no rate at all. Schemas cover names, not types: an unknown name can never work, while a wrong type comes back from the platform as a real error naming the field. The schema and the writer's allowlist must stay in step, and there is a test asserting they do.
- **Write methods take the location set, not a location.** `Create`/`Update`/`Delete` used to take a `locationID` *and* a `Resource` carrying `LocationIDs`, and the engine passed the first of forty IDs — a contract no adapter could implement correctly. The write side now reads `desired.LocationIDs` only; `Read`/`ReadAll` keep their `locationID`, which is genuinely read-side.
- **Cancellation means "unknown" on both write paths.** The batch path always reported an interrupted write as unknown rather than failed. The individual path returned a nil error, so `Apply` recorded every remaining resource as *failed* and sent the operator to re-create things that may already exist. `writeIndividually` now returns `ctx.Err()` with whatever outcomes did come back, and `recordOutcomes` matches outcomes to operations by name rather than position — because the individual path completes concurrently and may return fewer than it was given.
- **Stale-lock takeover goes through `O_EXCL`, like a first acquire.** Two processes can both judge the same lock dead. The takeover used to be a plain write, so both would "succeed" and two applies would run against one workspace. The loser is now told to wait.
...
- **A scope Square cannot express is an error, not a silent widening.** A category declared at two of forty locations cannot be written that way. Mise refuses the apply and names `${group.all}` rather than quietly putting it everywhere — the operator would otherwise see success and only discover the truth from the next fetch. `SquareProvider.checkLocationScope` does this, and caches the account's locations so a batch does not re-list them per operation.
- **The adapter knows which types Square can scope, and that list was measured.** `accountWideOnlyTypes` came from probing the sandbox with each type, not from the docs. Adding a resource type means checking it the same way — the failure mode is a 400 that names an attribute rather than the resource.
...
- **An item's category is `categories[]` + `reporting_category`, not `category_id`.** The old single-category field is *accepted and silently discarded* before API version `2024-06-04` — no error, the association simply never exists. This is why `APIVersion` is pinned to `2024-06-04` rather than something older. Reads still fall back to `category_id` so objects written earlier resolve; writes never send it.
- **`CATEGORY` cannot carry `present_at_location_ids`.** Square rejects it with "Unexpected unit-overridden attribute for definition present_at_all_locations". Categories are account-wide or nothing. Every other type Mise supports (tax, discount, item, modifier list) accepts an explicit list — verified against the sandbox, not assumed.
- **Nested objects inherit their parent's location scope.** An `ITEM_VARIATION` present at more locations than its `ITEM` is rejected with "enabled at all future locations, but the referenced object ... is not". Same for a `MODIFIER` inside a `MODIFIER_LIST`.
